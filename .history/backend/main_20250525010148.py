...
async def audio_request_generator(
    ...
    try:
        ...
        # This log is CRITICAL. If it doesn't appear, the error happens before or during the first yield.
        # If it DOES appear, the error happens when the library tries to get the *next* item.
        ...
        while not stop_event.is_set():
            ...
            message = await websocket.receive() # This is a key blocking point
            print(f"[{client_id}] GEN: Received WebSocket message raw: {message}") # Log the raw message
            ...
            if audio_chunk:
                print(f"[{client_id}] GEN: Yielding audio chunk of size {len(audio_chunk)}")
                yield StreamingRecognizeRequest(audio_content=audio_chunk)
            
            text_message = message.get('text')
            if text_message: # Handle text messages if any (e.g., control messages)
            ...
            # If message type is neither bytes, text, nor disconnect, it might be an issue
        ...
        # This catches WebSocketDisconnect if websocket.receive() raises it directly
        print(f"[{client_id}] GEN: WebSocketDisconnect exception caught directly in generator!")
        # stop_event.set() will be handled by the finally block
    except asyncio.CancelledError:
        print(f"[{client_id}] GEN: Audio request generator was cancelled.")
        # stop_event.set() will be handled by the finally block
    except Exception as e:
        # Catch any other unexpected exceptions from within the generator
        print(f"[{client_id}] GEN: Unexpected exception in generator: {type(e).__name__}: {e}")
        traceback.print_exc() # Print the full traceback for this unexpected error
        # stop_event.set() will be handled by the finally block
    finally:
        print(f"[{client_id}] GEN: Audio request generator finishing (stop_event: {stop_event.is_set()}).")
        if not stop_event.is_set(): # Ensure stop_event is set if generator exits for any reason

...
export default function App() {
  ...
  async function start() {
    ...
    try {
      ...
      // This is the original onmessage handler before timestamp/speaker changes
      ws.onmessage = (ev) => {
        const data = JSON.parse(ev.data);
        console.log("Received message from WebSocket:", data);
        setLines(prev => [
          ...prev.filter(line => line.isFinal), // Keep only previous final lines
          { text: (data.is_final ? "✔ " : "… ") + data.transcript, isFinal: data.is_final } // Add new line
        ]);
      };

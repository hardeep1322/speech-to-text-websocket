...
function App() {
  ...
  const startRecording = async () => {
    try {
      ...
      // Show the video track locally if you like
      if (videoRef.current) {
        videoRef.current.srcObject = displayStream
      }

      // ---- KEY CHANGE: keep only the audio track for the recorder ----
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0 && wsRef.current?.readyState === WebSocket.OPEN) {
          ...
          console.log(`Data available event with size ${event.data.size}, or WebSocket not open: ${wsRef.current?.readyState}`)
        }
      }
